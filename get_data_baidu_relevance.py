...
def extract_data(n, text):
    ...
    for page in range(n):
        ...
        # 准备下一页的URL
        url = url.replace(f'pn={page * 10}', f'pn={(page + 1) * 10}')

    return href_list


def scrape_publications(href_list):
    # 初始化DataFrame
    ...
    for i, href in enumerate(href_list):
        ...
        print(post_list2[0].h3.a.text.strip())
        # 提取标题
        # title = post_list2[0].h3.a.text.strip() \
        #     if post_list2[0].h3.a else "暂无"
        # 检查post_list2是否为空
        if post_list2:
            title = post_list2[0].h3.a.text.strip() if post_list2[0].h3.a else "暂无"
            print(title)
        else:
            print("未找到main-info类元素")
...
def summarize_data(data):
    ...
    for i in range(len(fold_data)):
        abstract.append(format_specific_columns(fold_data.iloc[i], ['Title', 'Abstract']))
        # 应用格式化函数并输出特定的列
...
# 假设您想要提取前1页的数据
href_list_want = extract_data(2, 'CNN')

# print(href_list)
row_data = scrape_publications(href_list_want)
row_data.to_csv('data.csv', encoding='utf-8-sig')
# 打印DataFrame
# print(df.shape[0])
