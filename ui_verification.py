...
from messages import fox_chat_chat, fox_say_read_paper, fox_say_read_paper_combine_with_analysis
...
# 以下是狐聊页面函数
def fox_chat(api_key, secrect_key, key, text, history, image, max_new_tokens, top_p, temperature):
    history.append((text, fox_chat_chat(api_key=api_key, secret_key=secrect_key,
                                        key=key, message=text, image_path=image,
...
with gr.Blocks(theme='rawrsor1/Everforest', js=js) as demo:
    with gr.Tab("狐聊"):
        with gr.Row():
            with gr.Column():
                with gr.Accordion("Key", open=False):
                    ernie_api_key = gr.Textbox(label="请输入文心一言的API Key", placeholder="请输入你的API Key",
                                               value="ZXwXDGuDYPtxj3Roanbj7LwH")
                    ernie_secret_key = gr.Textbox(label="请输入文心一言的API Secret Key",
                                                  placeholder="请输入你的Secret Key",
                                                  value="fRw3IZ1a0DAGHZh4g72NoGwwxLwA29xv")
                    llm_key = gr.Textbox(label="请输入图像处理的API Key", placeholder="请输入你的API Key",
        ...
        fox_chat_chat_input.submit(fn=fox_chat, inputs=[ernie_api_key, ernie_secret_key, llm_key, fox_chat_chat_input,
                                                        chat_box2, image_upload, max_new_tokens1, top_p1, temperature1],
    ...
    with gr.Tab("狐写"):
        with gr.Row():
            with gr.Column(scale=1):
                fox_write_input = gr.Textbox(label="你想让大狐狸写什么样的综述🔬？",
                                             placeholder="请输入您的消息并按回车↩")
            ...
            with gr.Column(scale=5):
                paper_summary_chat = gr.Textbox(label="来看看大狐狸写的综述，写的不好还请见谅。😣", lines=20,
                                                max_lines=20, show_copy_button=True)
